...
const CSP_POLICY = [
  ...
  "img-src 'self' data: blob: https://placehold.co https://images.unsplash.com https://*.tile.openstreetmap.org https://lh3.googleusercontent.com",
...
export function middleware(request: NextRequest) {
  ...
  // Rate limit: Seed/Setup endpoint (max 3 requests per hour per IP)
  if ((pathname === '/api/seed' || pathname === '/api/setup') && method === 'POST') {
    const key = getRateLimitKey(request, 'seed')
    if (isRateLimited(key, 3, 60 * 60 * 1000)) {
      return NextResponse.json(
        { error: 'Limite alcanzado. Espera 1 hora.', code: 'RATE_LIMITED' },
        { status: 429, headers: corsHeaders(request) }
      )
    }
  }
