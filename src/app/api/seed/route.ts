  ...
  try {
    ...
    const free = await db.plan.create({
      data: {
        ...
        features: JSON.stringify(['1 tienda online', '10 productos maximo', 'Plantilla basica', 'WhatsApp integrado', 'Soporte por email']),
        popular: false,
      },
    })
    ...
    const pro = await db.plan.create({
      data: {
        ...
        features: JSON.stringify(['1 tienda online', '50 productos maximo', 'Todas las plantillas', 'WhatsApp integrado', 'Estadisticas avanzadas', 'Soporte prioritario']),
        popular: true,
      },
    })
    ...
    const premium = await db.plan.create({
      data: {
        ...
        features: JSON.stringify(['Hasta 3 tiendas online', 'Productos ilimitados', 'Todas las plantillas', 'Dominio personalizado', 'WhatsApp Business API', 'Soporte 24/7', 'Estadisticas avanzadas', 'API access']),
        popular: false,
      },
    })
